            ...
            <tbody className="divide-y divide-gray-700">
                {items.map(item => (
                    <tr key={item.id}>
                        ...
                        <td className="p-2">{item.defaultPeriod || item.defaultAmount ? `${item.defaultPeriod || ''} - ${format(item.defaultAmount)}` : '-'}</td>
                    </tr>
                ))}
