...
import { CashAndCashEquivalentsData, ScheduleData, CashComponent } from '../../types.ts';
...
export const CashAndCashEquivalentsSchedule: React.FC<CashAndCashEquivalentsScheduleProps> = ({ data, onUpdate, isFinalized }) => {
    ...
    const handleFieldUpdate = (field: keyof CashAndCashEquivalentsData, value: string) => {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <InputField label="Cash on Hand" value={data.cashOnHand} onChange={v => handleFieldUpdate('cashOnHand', v)} disabled={isFinalized} />
                <InputField label="Cheques, Drafts on Hand" value={data.chequesDraftsOnHand} onChange={v => handleFieldUpdate('chequesDraftsOnHand', v)} disabled={isFinalized} />
            </div>
            ...
            <InputField label="Repatriation Restrictions" value={data.repatriationRestrictions} onChange={v => handleFieldUpdate('repatriationRestrictions', v)} disabled={isFinalized} />
        </div>
    );
};