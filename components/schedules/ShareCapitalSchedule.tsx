...
import { ShareCapitalData, ScheduleData, ShareCapitalItem, ShareReconciliationItem, Shareholder, PromoterShareholding } from '../../types.ts';
    ...
    return (
        <div className="space-y-6">
            ...
            <Section title="e. Other Disclosures">
                <DisclosureInput label="Rights, preferences and restrictions attaching to each class of shares" value={data.rightsPreferences} onChange={v => handleUpdate('rightsPreferences', v)} isFinalized={isFinalized} isTextarea />
                <DisclosureInput label="Shares held by holding company or ultimate holding company" value={data.holdingCompanyShares} onChange={v => handleUpdate('holdingCompanyShares', v)} isFinalized={isFinalized} />
                <DisclosureInput label="Aggregate number of shares allotted as fully paid up bonus shares (last 5 yrs)" value={data.fiveYearHistoryBonus} onChange={v => handleUpdate('fiveYearHistoryBonus', v)} isFinalized={isFinalized} />
                <DisclosureInput label="Aggregate number of shares allotted as fully paid up without payment in cash (last 5 yrs)" value={data.fiveYearHistoryNoCash} onChange={v => handleUpdate('fiveYearHistoryNoCash', v)} isFinalized={isFinalized} />
                <DisclosureInput label="Aggregate number of shares bought back (last 5 yrs)" value={data.fiveYearHistoryBuyback} onChange={v => handleUpdate('fiveYearHistoryBuyback', v)} isFinalized={isFinalized} />
                <DisclosureInput label="Terms of any securities convertible into equity/preference shares" value={data.convertibleSecurities} onChange={v => handleUpdate('convertibleSecurities', v)} isFinalized={isFinalized} />
                <DisclosureInput label="Calls unpaid (including details of directors/officers)" value={data.callsUnpaid} onChange={v => handleUpdate('callsUnpaid', v)} isFinalized={isFinalized} />
                <DisclosureInput label="Forfeited shares (amount originally paid-up)" value={data.forfeitedShares} onChange={v => handleUpdate('forfeitedShares', v)} isFinalized={isFinalized} />
            </Section>
        </div>
    );
};
...
const DisclosureInput: React.FC<{label:string, value:string, onChange:(v:string)=>void, isFinalized:boolean, isTextarea?:boolean}> = ({label, value, onChange, isFinalized, isTextarea}) => (
    <div>
        <label className="block text-xs font-medium text-gray-400">{label}</label>
        {isTextarea ? (
             <textarea value={value} onChange={e => onChange(e.target.value)} disabled={isFinalized} rows={2} className="mt-1 block w-full bg-gray-700/80 p-2 rounded-md text-sm"/>
        ) : (
             <input type="text" value={value} onChange={e => onChange(e.target.value)} disabled={isFinalized} className="mt-1 block w-full bg-gray-700/80 p-2 rounded-md text-sm"/>
        )}
    </div>
)
